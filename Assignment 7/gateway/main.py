...
DEFAULT_ORDER = ["ollama", "gemini", "gemini-lite", "gemini-35", "gemini-31-lite", "nvidia", "groq", "cerebras", "openrouter", "github"]
...
TIER_TO_ORDER = {
    "TINY":  ["github", "openrouter", "groq", "nvidia", "cerebras",
              "gemini-lite", "gemini-31-lite", "gemini", "gemini-35", "ollama"],
    "LARGE": ["gemini", "gemini-35", "gemini-lite", "gemini-31-lite",
              "groq", "nvidia", "cerebras", "github", "openrouter", "ollama"],
}
...
async def chat(req: ChatRequest):
    ...
    # When ALL candidates are blocked only by short-term cooldown (≤ 15s) —
    # not long backoffs — wait for the shortest cooldown rather than 503-ing.
    # This handles rapid sequential agent calls where every provider just
    # finished a call and is in its per-request cooldown window.
    else:
        name_check, _ = router.pick(est, candidates, required_caps=required_caps)
        if name_check is None:
            waits = []
            for c in candidates:
                snap = router.state[c].snapshot(LIMITS[c])
                back = snap["backoff_remaining"]
                cool = snap["cooldown_remaining"]
                if back > 15:
                    waits = []
                    break
                waits.append(max(back, cool))
            if waits:
                wait_secs = min(waits) + 0.1
                import asyncio as _asyncio
