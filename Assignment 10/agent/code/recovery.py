...
MAX_CRITIC_RECOVERIES = 2  # global cap across the entire run
...
def handle_critic_verdict(nid: str, result, graph, recovered_branches: dict,
                          cap_hit: list, critic_fail_count: list) -> bool:
    ...
    if target_nid and critic_fail_count[0] < MAX_CRITIC_RECOVERIES:
        critic_fail_count[0] += 1
        ...
        print(f"  ↪ critic-fail recovery: planner node {rec_nid} for {target_nid} "
              f"({critic_fail_count[0]}/{MAX_CRITIC_RECOVERIES})")
    elif target_nid:
        cap_hit.append(target_nid)
        print(f"  ↪ critic-fail global cap ({MAX_CRITIC_RECOVERIES}) hit — "
              f"branch skipped, final will use best available data")
    return True
