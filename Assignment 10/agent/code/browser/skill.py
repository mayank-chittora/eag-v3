"""Browser skill — cascade wrapper around the layered drivers.
...
_UA = (
    "Mozilla/5.0 (compatible; Browser-Skill/0.1; +llm_gateway)"
)
    ...
    async def run(self, node: NodeSpec) -> AgentResult:
        ...
        if force_path == "vision":
            ...
            try:
                ...
                # Playwright can raise "Execution context was destroyed" when
                # a page navigates mid-evaluation. Catch it here so the skill
                # returns a structured failure instead of an uncaught exception
                # that would make the orchestrator record elapsed_s=0.0.
                a11y_result = DriverResult(success=False, note=f"a11y driver exception: {e}")
        ...
        try:
            ...
            vis_result = DriverResult(success=False, note=f"vision driver exception: {e}")
