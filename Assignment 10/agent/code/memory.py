...
The classifying write for ambiguous free-form content uses one gateway
chat call routed `auto_route="memory"`. The embedding call is a separate
gateway endpoint, `POST /v1/embed`, exposed by `gateway.embed()`.

Design notes:
  1. Vector retrieval only. Hybrid retrieval with RRF is not yet
     implemented.
...
# Minimum cosine similarity for a vector hit to be returned. Items below
# this threshold are off-topic (e.g. laptop memories surfacing in a
# HuggingFace query) and should be silently dropped rather than injected
# into every skill's context.
