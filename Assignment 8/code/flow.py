...
The agent's loop is a NetworkX DiGraph. Each node is a skill; edges carry
typed AgentResult payloads. The graph GROWS at runtime via five actors:
the Planner's seed plan, dynamic successors from any skill, static
`internal_successors` from the yaml, Critic auto-insertion on edges out
of `critic:true` skills, and Planner re-invocation on node failure
(gated by `recovery.plan_recovery`). The Planner names skills, never
tools.
...
class Graph:
    ...
    def extend_from(self, src_nid: str, result: AgentResult,
        ...
        # Also guard pre-planned edges: when the planner wired up a successor
        # to this critic-flagged skill before it ran, those edges were never
        # seen by the block above (added was empty or only held dynamic nodes).
        # Walk current direct successors that are not already critic nodes and
        # intercept them the same way.
        if src_def.critic:
            pre_planned = [
                child for child in list(self.g.successors(src_nid))
                if self.g.nodes.get(child, {}).get("skill") != "critic"
            ]
            for child_nid in pre_planned:
                self.g.remove_edge(src_nid, child_nid)
                critic_nid = self.add_node(
                    "critic", inputs=[src_nid],
                    metadata={"target": src_nid, "child": child_nid},
...
class Executor:
    ...
    async def run(self, query: str, *, session_id: str | None = None,
        ...
        # skill's prompt so every cognitive role can see what the agent
        # already knows before deciding what work to schedule.
