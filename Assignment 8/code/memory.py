...
Reads go through FAISS first (cosine similarity over the `embedding` field).
When the vector path returns nothing, the read falls back to keyword overlap.
...
The classifying write for ambiguous free-form content uses one gateway chat
call routed `auto_route="memory"`. The embedding call is a separate gateway
endpoint, `POST /v1/embed`, exposed by `gateway.embed()`.

Three design choices to be aware of:
  1. Vector retrieval only. Hybrid retrieval with RRF is not yet implemented.
...
def _index() -> VectorIndex:
    ...
    Re-reading the index file is cheap and keeps the agent
