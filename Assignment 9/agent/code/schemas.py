"""Typed contracts shared across all agent layers.
...
`MemoryItem` carries an optional `embedding` field. Items of kind `fact`,
...
# Structured failure taxonomy for the Browser skill. Other skills leave
# error_code=None and fall through to recovery.classify_failure's text
...
class AgentResult(BaseModel):
    ...
    provider: str = ""
    error: str | None = None
    # Structured failure code for the Browser skill (other skills leave it
    # None and fall through to recovery's text heuristics).
...
class NodeState(BaseModel):
    ...
    prompt_sent: str | None = None   # exact bytes sent to the gateway; used by replay
