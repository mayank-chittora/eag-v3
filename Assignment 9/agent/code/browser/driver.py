...
class BaseDriver:
    ...
    async def step(self, turn: int) -> tuple[bool, bool, str]:
        # enumerate_interactives calls page.evaluate(); if the page is mid-navigation
        # (JS redirect, React hydration, Turbo) the execution context is destroyed.
        # Wait briefly and retry once — after navigation settles the context is stable.
        try:
            snap = await enumerate_interactives(self.page)
        except Exception as _e:
            _emsg = str(_e)
            if "Execution context" in _emsg or "navigation" in _emsg.lower() or "Target closed" in _emsg:
                await asyncio.sleep(2.0)
                try:
                    snap = await enumerate_interactives(self.page)
                except Exception as _e2:
                    rec = StepRecord(
                        turn=turn, thinking="", actions=[],
                        outcome=f"error: evaluate failed after retry: {_e2}",
                        provider="", model="", latency_ms=0,
                        tokens_in=0, tokens_out=0,
                    )
                    self.steps.append(rec)
                    return False, False, str(_e2)
            else:
                raise
