"""Growing-graph multi-agent orchestrator.
...
failure (gated by `recovery.plan_recovery`). The tool-blindness
...
class Executor:
    ...
    async def run(self, query: str, *, session_id: str | None = None,
        ...
        # Global counter so the cap fires even when each critic targets a
        # different (freshly created) node ID.
        critic_fail_count: list[int] = [0]
        # Track every capped critic-fail so the final log can surface it.
