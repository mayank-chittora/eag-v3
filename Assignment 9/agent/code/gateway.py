...
The gateway exposes: (1) `/v1/vision` — typed shim for single-image
...
addition to tokens; and (3) `agent` tagging, `/v1/chat/batch`, and
retry-on-5xx.
...
GATEWAY_V9_DIR = Path(__file__).resolve().parents[2] / "gateway"
...
def ensure_gateway() -> None:
    ...
    if not GATEWAY_V9_DIR.exists():
        raise RuntimeError(
            f"Gateway directory not found at {GATEWAY_V9_DIR}. "
            "Ensure the gateway/ directory exists before running this code."
        )
