...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    orchestrator delivers the hits, not just because the FAISS index is
    on disk.
    ...
    if skill.name == "browser":
        ...
        # The Planner sometimes writes metadata.url = "n:<label>" intending
        # it as a node-output reference. Metadata values are NOT resolved by
        # the graph's input system (only inputs[] are), so we resolve them
        # here before passing to the browser skill — otherwise Playwright
        # tries page.goto("n:repo_urls") and raises ERR_FILE_NOT_FOUND.
        url_meta = metadata.get("url", "")
        if url_meta.startswith("n:"):
            _url_resolved = resolve_inputs([url_meta], graph_nodes, query)
            if _url_resolved:
                _r = _url_resolved[0]
                if _r.get("kind") == "upstream" and isinstance(_r.get("output"), dict):
                    _out = _r["output"]
                    _found = (_out.get("url") or _out.get("final_url")
                              or _out.get("href") or "")
                    if str(_found).startswith("http"):
                        metadata["url"] = _found
                    else:
                        metadata.pop("url", None)
                else:
                    metadata.pop("url", None)
        # If the URL is a bare base domain (e.g. "https://github.com/") OR contains
        # a placeholder path (e.g. "https://github.com/placeholder2", "/repo1") the
        # Planner used a template URL. Extract the nth real URL from upstream content
        # based on ordinal words in the goal ("first", "second", "third").
        _cur_url = metadata.get("url", "")
        if _cur_url and not _cur_url.startswith("n:"):
            import re as _re
            from urllib.parse import urlparse as _urlparse
            _pp = _urlparse(_cur_url)
            _path_lower = _pp.path.lower().strip("/")
            _is_placeholder = (
                _pp.path in ("", "/")
                or "placeholder" in _path_lower
                or _re.match(r'^repo\d*$', _path_lower)
                or _re.match(r'^url\d*$', _path_lower)
            )
            if _pp.netloc and _is_placeholder:
                _goal_lower = (metadata.get("goal") or "").lower()
                _ord_map = {"first": 1, "1st": 1, "second": 2, "2nd": 2,
                            "third": 3, "3rd": 3, "fourth": 4, "4th": 4}
                _nth = next((v for k, v in _ord_map.items() if k in _goal_lower), 1)
                _base = f"{_pp.scheme}://{_pp.netloc}"
                # Collect ALL candidates across all upstream sources (trafilatura
                # typically only emits a markdown link for the first item; gather
                # everything so nth-item lookups for repo2/repo3 can succeed).
                _all_candidates: list = []
                for _r in resolve_inputs(node_dict.get("inputs") or [], graph_nodes, query):
                    if _r.get("kind") != "upstream":
                        continue
                    _content = str((_r.get("output") or {}).get("content") or "")
                    if not _content:
                        continue
                    # markdown-style relative paths (/owner/repo) from trafilatura
                    _rel = _re.findall(
                        r'\((/[A-Za-z0-9][A-Za-z0-9_.-]*/[A-Za-z0-9][A-Za-z0-9_.-]*)\)',
                        _content,
                    )
                    # full URLs on same domain: https://github.com/owner/repo
                    _full = _re.findall(
                        r'https?://' + _re.escape(_pp.netloc)
                        + r'/([A-Za-z0-9][A-Za-z0-9_.-]*/[A-Za-z0-9][A-Za-z0-9_.-]*)',
                        _content,
                    )
                    _all_candidates += [f"{_base}{p}" for p in _rel]
                    _all_candidates += [f"{_base}/{p}" for p in _full]
                _seen_c: set = set()
                _deduped = []
                for _u in _all_candidates:
                    if _u not in _seen_c:
                        _seen_c.add(_u)
                        _deduped.append(_u)
                if len(_deduped) >= _nth:
                    metadata["url"] = _deduped[_nth - 1]
                else:
                    # Could not resolve a real URL — drop the placeholder so the
                    # browser returns "no url given" cleanly instead of hitting a
                    # fake path that triggers gateway_blocked or ERR_FILE_NOT_FOUND.
                    metadata.pop("url", None)
    ...
    # dropped, which left callers chasing "missing node" bugs for an hour.
